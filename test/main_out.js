...
class BinaryReader {
    constructor(view) {
        ...
        this.offset = 0;
        this.error = false; // Флаг ошибки
    }

    get canRead() {
        return this.offset < this.byteLength && !this.error;
    }

    safeRead(callback, size) {
        try {
            if (this.offset + size > this.byteLength) throw new Error("Out of bounds");
            const value = callback();
            this.offset += size;
            return value;
        } catch (e) {
            console.warn("BinaryReader: Ошибка чтения, сообщение пропущено.", e);
            this.error = true;
            return null;
        }
    }

    uint8() { return this.safeRead(() => this.view.getUint8(this.offset), 1); }
    int8() { return this.safeRead(() => this.view.getInt8(this.offset), 1); }
    uint16() { return this.safeRead(() => this.view.getUint16(this.offset, true), 2); }
    int16() { return this.safeRead(() => this.view.getInt16(this.offset, true), 2); }
    uint32() { return this.safeRead(() => this.view.getUint32(this.offset, true), 4); }
    int32() { return this.safeRead(() => this.view.getInt32(this.offset, true), 4); }

    utf16() {
        let str = "", char;
        while (this.canRead && (char = this.uint16()) !== 0 && char !== null) {
...
function handleWsMessage(msg) {
    ...
    function getString() {
        ...
        try {
            ...
            console.warn("getString: повреждённая строка");
        }
        return text;
    }
    ...
    let messageType;
    try {
        messageType = msg.getUint8(offset++);
    } catch (e) {
        console.warn("handleWsMessage: повреждён тип сообщения");
        return;
    }
    ...
    try {
        switch (messageType) {
            case 16: {
                ...
                reader.offset++; // пропускаем messageType
                updateNodes(reader);
                if (reader.error) console.warn("Ошибка в updateNodes, пропущено.");
                break;
            }
            ...
            case 21:
                try {
                    lineX = msg.getInt16(offset, true); offset += 2;
                    lineY = msg.getInt16(offset, true); offset += 2;
                    if (!drawLine) {
                        drawLine = true;
                        drawLineX = lineX;
                        drawLineY = lineY;
            ...
            case 49:
                try {
                    ...
                    const LBplayerNum = msg.getUint32(offset, true); offset += 4;
                    leaderBoard = [];
                    for (let i = 0; i < LBplayerNum; ++i) {
                        const nodeId = msg.getUint32(offset, true); offset += 4;
                        const playerName = getString();
                        const playerXp = msg.getUint32(offset, true); offset += 4;
                        const level = playerXp ? getLevel(playerXp) : -1;
                        leaderBoard.push({ id: nodeId, name: playerName, level });
                    }
                    drawLeaderBoard();
                } catch (e) {
                    console.warn("Ошибка в таблице лидеров", e);
                }
                break;
            ...
            case 50:
                try {
                    teamScores = [];
                    const LBteamNum = msg.getUint32(offset, true); offset += 4;
                    ...
                    console.warn("Ошибка в командной таблице");
                }
                break;

            case 64:
                try {
                    leftPos = msg.getFloat64(offset, true); offset += 8;
                    topPos = msg.getFloat64(offset, true); offset += 8;
                    rightPos = msg.getFloat64(offset, true); offset += 8;
                    bottomPos = msg.getFloat64(offset, true); offset += 8;
                    foodMinSize = Math.sqrt(msg.getUint16(offset, true) * 100); offset += 2;
                    foodMaxSize = Math.sqrt(msg.getUint16(offset, true) * 100); offset += 2;
                    ...
                    console.warn("Ошибка в границах карты");
                }
                break;
            ...
            case 99:
                try {
                    ...
                    console.warn("Ошибка в сообщении чата");
                }
                break;
            ...
            case 114:
                try {
                    ...
                    console.warn("Ошибка в XP");
                }
                break;
        }
    } catch (e) {
        console.warn("handleWsMessage: ошибка при разборе сообщения", e);
        // Не выкидываем игрока, просто пропускаем сообщение
    }
}
