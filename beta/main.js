(function (global) {
  ...
  fontWeight: '700',        // ← критично, ты грузишь жирный
  fontSize: 100,
  lineJoin: "round",
  fill: "white",
  stroke: "black",
  ...
  if (this.core?.skins) this.core.skins.applyToCell(this, value);
...
connect(addr, passedToken) {
  ...
  ws.onerror = this.onError;
        ...
        spawn() {
            ...
            writer.setStringUTF16(this.core.store.name)
            this.send(writer)
        }
        ...
        onSpectateCamera(reader) {
            this.core.app.camera.target.s = 0.15;
        }
...
this.foodMaxSize = Math.sqrt(reader.getUint16() * 100);
  ...
  if (cellsByID.has(id)) {
    ...
    if (color) cell.color = color; // tint
    if (name)  cell.name  = name;  // имя/скин (если добавишь)
  } else {
    // новая клетка
...
drawBackground() {
  ...
  this.backgroundSprite = bg;
        ...
        initRenderer() {
            ...
            virusRenderTexture.baseTexture.mipmapMode = PIXI.MIPMAP_MODES.ON
        ...
        loop(now = performance.now()) {
            ...
            if (dt >= this._fpsUpdateMs) {
                this.core.stats.fps = (this._fpsFrames * 5000) / dt;
                this._fpsFrames = 0;
                this._fpsLast = now;
            }
...
updateCamera() {
  ...
  if (cells.length > 0) {
    ...
    for (const cell of cells) {
      // в old main_out score считался по r^2/100 — оставим как было у тебя
      score += ~~(cell.r * cell.r / 100);
      x += cell.x; y += cell.y;
      sumR += cell.r; // как old: суммируем size (= radius)
    }
    this.camera.target.x = x / cells.length;
    this.camera.target.y = y / cells.length;

    // тот же сглаженный viewZoom, что и в old main_out
    this.calcViewZoom(sumR);
  } else {
    // без клеток — потихоньку затухаем масштаб к текущему (ничего не рвём)
    this.viewZoom = (9 * this.viewZoom + this.viewRange()) / 10;
  }

  // позиция камеры — как у тебя (мягкое следование)
  this.camera.x += (this.camera.target.x - this.camera.x) / 2;
  this.camera.y += (this.camera.target.y - this.camera.y) / 2;

  // масштаб камеры теперь = viewZoom из формулы old main_out
  this.camera.target.s = this.viewZoom;
  this.camera.s += (this.camera.target.s - this.camera.s) / 2;
  ...
  this.camera.score = score;
    ...
    class UserInterface {
        ...
        addEvents() {
            ...
            window.view.addEventListener("mousedown", () => {
                // Owned player count 0 -> is spectate or dead
                if (!CORE.app.ownedCells.length) { // Update spectate position
                    updateMouseAim();
                    CORE.net.sendUint8(1);
                }
            });
        ...
        updateLeaderboard() {
            ...
            for (const player of leaderboard) {
                contentStr += `<div class="hud-leaderboard-tile ${CORE.app.ownedCells.some(id => id === player.id) ? "red-text" : ""}">${player.name}</div>`
            }
...
onScroll({ deltaY }) {
  ...
  const steps = (deltaY || 0) / 120;   // ← инвертировали

  app.zoom *= Math.pow(0.9, steps);

  if (app.zoom < 0) app.zoom = 1;
  if (app.zoom > 4 / app.viewZoom) app.zoom = 4 / app.viewZoom;
  if (app.zoom < 0.04) app.zoom = 0.04;
}
...
async init() {
  ...
  const url = `ws${location.protocol === 'https:' ? 's' : ''}://${Object.keys(servers)[0]}`;
