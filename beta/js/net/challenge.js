export function getGameServerApiBase(hostOrUrl) {
    if (!hostOrUrl) return "https://reg.agar.su";
    if (/^https?:\/\//i.test(hostOrUrl)) return hostOrUrl.replace(/\/$/, "");
    const host = String(hostOrUrl).replace(/^wss?:\/\//i, "").split("/")[0].split("?")[0];
    return "https://" + host;
}
...
export async function fetchConnectToken(gameHost) {
