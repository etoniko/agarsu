...
export function hexStrokeTint(color) {
    const base = String(color || "#FFFFFF");
    const parseChannel = (i) => {
        const part = base.length >= i + 2 ? base.substr(i, 2) : "00";
        let c = Math.floor(parseInt(part, 16) * 0.9);
        return (c < 16 ? "0" : "") + c.toString(16);
    };
    return parseInt(`${parseChannel(1)}${parseChannel(3)}${parseChannel(5)}`, 16);
}
