...
import { drawCellGL } from '../render/cell-draw-gl.js';
  ...
  function canvasResize(S) {
    ...
    if (S.uiCanvas) {
      S.uiCanvas.width = S.canvasWidth;
      S.uiCanvas.height = S.canvasHeight;
      S.uiCanvas.style.width = `${wHandle.innerWidth}px`;
      S.uiCanvas.style.height = `${wHandle.innerHeight}px`;
    }
    if (S.glRenderer) {
      S.glRenderer.resize(S.canvasWidth, S.canvasHeight);
    }
  }
  var deps = {
    S: null,
  ...
  function drawGameScene() {
    const S = deps.S;
    if (!S || (!S.ctx && !S.glRenderer)) return;
    ...
    if (S.glRenderer) {
      const glr = S.glRenderer;
      glr.beginWorld(nodeX, nodeY, viewZoom);
      drawCustomMapBackground(null);
      for (let i = 0; i < S.nodelist.length; i++) {
        S.nodelist[i].drawOneCell(null);
      }
      glr.flushWorld();
      glr.flushScreen();
    } else if (ctx) {
  ...
  function drawGradientGrid() {
    ...
    if (S.glRenderer) {
      S.glRenderer.drawGradientBackground(centerColor, edgeColor, gx, gy, gr);
      return;
    }
    if (!ctx) return;
  ...
  function drawClassicGrid(bgColor, lineColor) {
    ...
    const {ctx, canvasWidth, canvasHeight, viewZoom, nodeX, nodeY} = S;
    if (S.glRenderer) {
      S.glRenderer.drawClassicBackground(bgColor, lineColor, nodeX, nodeY, viewZoom);
      return;
    }
    if (!ctx) return;
  ...
  function drawCustomMapBackground(ctx) {
    ...
    if (S.glRenderer) {
      S.glRenderer.drawMapBackground(
        S.mapBgImage,
        left,
        top,
        mapW,
  ...
  Cell.prototype = {
    ...
    drawOneCell(ctx) {
      ...
      if (S.glRenderer) {
        ...
        drawCellGL(S.glRenderer, this, S, {
          getSkinImage: deps3.getSkinImage || getSkinImage,
          loadCachedImage: deps3.loadCachedImage || loadCachedImage,
          normalizeNick: deps3.normalizeNick || normalizeNick,
          getStickerUrl
        });
        return;
      }
  ...
  function attachInput(S, hooks) {
    ...
    S.uiCanvas = document.getElementById("canvas-ui");
    S.glRenderer = tryCreateWebGL2Renderer(S.mainCanvas);
    if (S.uiCanvas) {
      S.ctx = S.uiCanvas.getContext("2d");
      S.uiCanvas.style.pointerEvents = "none";
    } else if (!S.glRenderer) {
