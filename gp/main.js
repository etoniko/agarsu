(() => {
  ...
  function readServersFromHtml() {
    ...
    document.querySelectorAll(".server-item[data-ip]").forEach(item => {
      ...
      if (!key) return;
      if (!regions[region]) regions[region] = { label: region === "nl" ? "Нидерланды" : "Россия", available: true, servers: {} };
  ...
  function getDefaultRegion() {
    try { const saved = localStorage.getItem("agar_region"); if (saved && REGION_CONFIGS[saved]) return saved; } catch (e) {}
    const browserLanguage = String((navigator && (navigator.language || navigator.userLanguage)) || "").toLowerCase();
    return browserLanguage.startsWith("ru") ? "ru" : "nl";
  }
  ...
  function createGameState() {
    return {
      ...
      ws: null,
      wsUrl: null,
      CONNECTION_URL: "ffa.agar.su:443",
...
function updateRegionOnlineTotals(totals) {
  ...
  function installGlobalRatingHome(S) {
    ...
    function loadGlobalRatingHome() {
      fetch(STATS_API + "/rankings?limit=3", {
        cache: "no-store"
...
function parseServerHash(hashValue) {
  ...
  if (REGION_CONFIGS.nl?.servers[value]) return { regionKey: "nl", serverKey: value };
