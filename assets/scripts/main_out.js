...
async function runPowVerification() {
    ...
    const nonce = await solveConnectChallenge(challenge);
    setConnectVerifyStage(65, "Подтверждение…");
...
function connectGameserver(wsUrlArg) {
    const c = CONNECTION_URL;
    wsUrl = wsUrlArg || getGameServerWssUrl(c);
    const qs = new URLSearchParams();
    if (localStorage.accountToken) qs.set("accountToken", localStorage.accountToken);
    ws = new WebSocket(wsUrl + (qs.toString() ? "?" + qs.toString() : ""), "eSejeKSVdysQvZs0ES1H");
...
function showConnecting() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN && currentWebSocketUrl === wsUrl) return;
    if (ma) {
...
async function joinServer(wsUrlArg) {
    if (connectInProgress) return;
    connectInProgress = true;
    hideBanBanner();
    showConnectVerifyOverlay("Подключение…");
    ...
    try {
        await runPowVerification();
    } catch (err) {
        console.error("PoW error:", err);
        connectInProgress = false;
        setConnectVerifyStage(0, "Ошибка проверки, повтор…");
        return;
    }

    setConnectVerifyStage(90, "Подключение к gameserver…");
    connectGameserver(wsUrlArg);
    connectInProgress = false;
}

async function wsConnect(wsUrlArg) {
    await joinServer(wsUrlArg);
}
...
function drawLeaderBoard() {
  ...
  if (leaderBoard && leaderBoard.length > 0) {
    ...
    wHandle.connect = joinServer;
    wHandle.pow = runPowVerification;
