...
setInterval(fetchSkinList, 5000); // Проверяем каждые 60 секунд
...
async function chekstats() {
  ...
  let LOAD_RECAPTCHA = false;
  let type_global_recaptcha = null;
  let siteKey = '6LeRi80pAAAAAN5fJUIpD8QA5Ogoj1ppy70mwKOI';
  let recaptcha_global = null;
...
$(document).ready(function() {
  ...
  function onTouchStart(e) {
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      if ((touch.clientX * dpr > canvasWidth - size) && (touch.clientY * dpr > canvasHeight - size)) {
        sendMouseMove();
        sendUint8(17); // split
      }

      if ((touch.clientX * dpr > canvasWidth - size) && (touch.clientY * dpr > canvasHeight - 2 * size - 10) && (touch.clientY * dpr < canvasHeight - size - 10)) {
        sendMouseMove();
        sendUint8(21); // eject
      }
    }
  ...
  function onTouchMove(e) {
    ...
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      if (leftTouchID == touch.identifier) {
        ...
        rawMouseX = leftVector.x * dpr * 3 + canvasWidth / 2;
        rawMouseY = leftVector.y * dpr * 3 + canvasHeight / 2;
        mouseCoordinateChange();
        sendMouseMove();
      }
    }
...
function handleWheel(event) {
    ...
    if (overlay.is(':visible')) {
        return; // Stop zooming if the overlay is open
    }
  ...
  function handleWsMessage(msg) {
    ...
    switch (messageType) {
      ...
      case 114:
        // Handle recaptcha
        offset = 5;
        const typeRecaptcha = getString();

        console.log(typeRecaptcha)

        if (typeRecaptcha === 'recaptcha_v3') {
          type_global_recaptcha = typeRecaptcha;

          grecaptcha.ready(function () {
            grecaptcha.execute('6LeHi80pAAAAAEqWEsZ5tSrcQ53xJfxXI1SwUKQK', { action: 'submit' }).then(function (token) {
              recaptchaSend(token);
            });
          });
        } else if (typeRecaptcha === 'recaptcha_v2') {
          wjQuery('#recaptcha').show();
          type_global_recaptcha = typeRecaptcha;
          grecaptcha.reset(recaptcha_global);
        } else if (typeRecaptcha === 'error') {
          alert('We cannot identify you as a person.');
          type_global_recaptcha = typeRecaptcha;
          grecaptcha.reset(recaptcha_global);
          wjQuery('#recaptcha').hide();
          ws.close();
        } else if (typeRecaptcha === 'success_v2') {
          wjQuery('#recaptcha').hide();
        }
        break;
      default:
        console.log('Unknown packet type:', messageType);
        break;
    }
  }
  ...
  const loadRecaptcha = function () {
    ...
    recaptcha_global = grecaptcha.render('recaptcha-global', {
      'sitekey': siteKey,
      'theme': 'dark',
      'callback': recaptchaSend
    });
  };

  function setStringToBuffer(msg, str, offset) {
  ...
  function recaptchaSend(val) {
    const textTypeRecaptcha = type_global_recaptcha || "";
    const totalLength = 9 + (2 * val.length) + (2 * textTypeRecaptcha.length);
    const msg = prepareData(totalLength);
    msg.setUint8(0, 113);

    let offset = 5;

    offset = setStringToBuffer(msg, val, offset);
    setStringToBuffer(msg, textTypeRecaptcha, offset);
...
function drawGameScene() {
    if (!LOAD_RECAPTCHA && typeof grecaptcha != 'undefined') {
    ...
    if (touchable) {
      for (var i = 0; i < touches.length; i++) {
        ...
        if (touch.identifier == leftTouchID) {
          ...
          ctx.lineWidth = 6;
          ctx.arc(leftTouchStartPos.x, leftTouchStartPos.y, 80, 0, Math.PI * 2, true);
          ctx.stroke();
          ctx.beginPath();
          ctx.strokeStyle = "#0096ff";
          ctx.lineWidth = 2;
          ctx.arc(leftTouchStartPos.x, leftTouchStartPos.y, 120, 0, Math.PI * 2, true);
          ctx.stroke();
          ctx.beginPath();
          ctx.strokeStyle = "#0096ff";
          ctx.arc(leftTouchPos.x, leftTouchPos.y, 80, 0, Math.PI * 2, true);
          ctx.stroke();
        }
      }
    }
