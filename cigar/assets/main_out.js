...
const getLevel = xp => ~~((xp / 100 * 2) ** 0.5);
    var CONNECTION_URL = "ffa.agar.su:6001", // Default Connection
...
function drawGameScene() {
    ...
    // ────────────────────────────────────────────────
    // Добавляем подсчёт FPS (вставляем сюда)
    ...
    let displayText = '';
    if (userScore > 0) {
        displayText += 'Score: ' + ~~(userScore / 100);
    }
    ...
    if (displayText) {
        ...
        scoreText.setValue(displayText);
        let rendered = scoreText.render();
        ...
        // Можно дополнительно покрасить только FPS (опционально)
        // Но для простоты оставляем белым, как Score
    }
    // ────────────────────────────────────────────────
